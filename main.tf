...
resource "ibm_is_instance" "instance1daniel" {
  name    = "instance1daniel"
  image   = var.image
  profile = var.profile
...
resource "ibm_is_instance" "instance2daniel" {
  name    = "instance2daniel"
  image   = var.image
  profile = var.profile
  primary_network_interface {
...
resource "ibm_is_instance" "instance3daniel" {
  name    = "instance3daniel"
  image   = var.image
  profile = var.profile
  primary_network_interface {
...
resource "ibm_is_floating_ip" "floatingip3" {
  name = "fip3"
  target = "${ibm_is_instance.instance3daniel.primary_network_interface.0.id}"
}
...
resource "ibm_is_security_group_rule" "sg1_tcp_rule_80" {
  ...
  port = "80"
  target_address = "${ibm_is_instance.instance3daniel.primary_network_interface.0.primary_ipv4_address}"
}
}
